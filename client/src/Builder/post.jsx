...
import lodash, { includes, isNull, set, transform } from "lodash";
...
const Post = ({post,setPost,handleSubmit,mainTheme,setIsEditPost,setIsAddPost}) => {
...
useEffect(()=>{
  ...
  if(post.category.length === 0){
    handleChange(event)
  }else if(post.category.length > 1 && post.category.includes("-")){
    let cates = lodash.cloneDeep(post.category)
  ...
  "& .MuiInputLabel-root.Mui-disabled": { color: "whitesmoke !important" },
        ...
        >
          PX
                    ...
                    <Button sx={{backgroundColor:"#454b57",color:"white",minWidth:38,height:35,borderStyle:"solid",borderColor:"#5e636d",borderWidth:"1px"}} onClick={()=>setNewPost(prev=>{
        ...
        <FormControl fullWidth variant="outlined" sx={COMMON_FIELD_SX}>
          ...
          <Select
            ...
            renderValue={(selected) => {
              const isNoCate = post.category.includes("-")
              return isNoCate
          ...
          >
            {cates.map((cate,i)=>{
              ...
              return( <MenuItem value={cate} key={i} sx={{ fontSize: 15 }} hidden={post.category.length === 1 && post.category[0] === "-"}>
              <Checkbox checked={post.category.includes(cate)}/>
