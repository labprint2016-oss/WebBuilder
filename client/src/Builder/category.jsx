...
import { listCates,createCate,deleteCate ,editCate} from "../../Functions/cate";
...
export default function EnhancedTable({copy,setIsEditPost,setPostID}) {
  ...
  const [editCateData,setEditCate] = useState({
    title:"",
    image:null
  })
...
function setRowData() {
  ...
  cates.forEach((cate, index) => {
    const newRow = {};
    newRow.image = cate.image;
    newRow.title = cate.title
    newRow.id = cate._id;
    newRows.push(newRow);
  });
  return newRows;
...
useEffect(() => {
  ...
  const [openImg,setOepnImg] = useState(false)
  ...
  const hadleSubmit = (e)=>{
    ...
    for(let [key,value] of Object.entries(newCate)){
  ...
  const hadleEdit = (e)=>{
    e.preventDefault()
    const id = editCateData.id
    if(typeof editCateData.image === "string"){
      editCate(id,editCateData)
    .then(res=>{
      loadCates()
      setIsEdit(false)
      setEditCate({
      ...
      const formData = new FormData()
      for(let [k,v] of Object.entries(editCateData)){
        if(k !== "id" ) formData.append(k,v)
      }
      editCate(id,formData)
      .then(res=>{
        loadCates()
        setIsEdit(false)
        setEditCate({
  ...
  const btns = [{key:"copy",Icon:Copy,def:(cate)=>{
    setIsEdit(true)
    setEditCate(cate)
  }}, {key:"edit",Icon:FilePenLine,def:(cate)=>{
    setIsEdit(true)
    setEditCate(cate)
  }}];
  ...
  const emptyRows =
    ...
    const getBtnText = ()=>{
      let text = "อัปโหลด"
      if(isEdit && typeof editCateData.image === "string"){
        text = "มีรูปภาพแล้ว"
      }else if((isEdit && typeof editCateData.image !== "string") || (!isEdit && newCate.image)){
        text += "แล้ว"
      }
      return text
    }
  ...
  >
        ...
        />
         ...
         <Button
          ...
          >
            ...
            <div className="text-white">{getBtnText()}</div>
          </Button>
    ...
    {rows.length > 1 && (
      ...
      <col style={{ width: "auto",minWidth:300 }} />    
      <col style={{ width: 250,minWidth:160 }} /> 
    </colgroup>
            <EnhancedTableHead
  ...
  <Box sx={{ display: "flex", justifyContent: "center" }}>
    ...
    />
        ...
        />
              ...
              <ImageModal openModal={openImg} setOpenModal={setOepnImg} handleChange={(img)=>{
                console.log(img);
                if(isEdit){
                  setEditCate((prev)=>{
                    const newData = {...prev}
