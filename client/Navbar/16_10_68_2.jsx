...
function Navbar({ handleDragElement, isDark }) {
  ...
  const [data, setData] = useState({
    Elements: [
      { label: "Column", icon: "dashboard_customize" ,id:"column"},
      { label: "Menu", icon: "reset_tv" ,id:"menu"},
      { label: "Header", icon: "center_focus_weak" ,id:"header"},
      { label: "Image", icon: "photo_size_select_actual",id:"image" },
      { label: "Heading", icon: "auto_awesome",id:"heading" },
      { label: "Text", icon: "format_size",id:"text"},
      { label: "Button", icon: "link",id:"button" },
      { label: "Button Dual", icon: "smart_button" ,id:"buttonDual"},
      { label: "iCons", icon: "sentiment_very_satisfied",id:"icons" },
      { label: "Carousel", icon: "more_horiz" ,id:"carousel"},
      { label: "Gallery", icon: "add_photo_alternate" ,id:"gallery"},
      { label: "Youtube", icon: "slow_motion_video",id:"youtube" },
      { label: "List iTem", icon: "storage",id:"listItem" },
      { label: "Table", icon: "table_chart" ,id:"table"},
      { label: "Form", icon: "forward_to_inbox" ,id:"form"},
      { label: "Card", icon: "confirmation_number",id:"card" },
      { label: "Divider", icon: "insert_page_break",id:"divider" },
      { label: "Footer", icon: "power_input",id:"footer" },
    ],
    Theme: {
      headingOptions: [
  ...
  const DragItem = ({ elementType, icon ,id}) => {
    const { attributes, listeners, setNodeRef } = useSortable({
      id,
      data: { icon,elementType ,isSidebarElement: true},
    });
    return (
      <div
        {...attributes}
          ...
          <ul className="mt-1 pl-1">
            <li>
              {selectedMenuId === "Elements" && (
                <SortableContext items={data.Elements.map(i=>i.label)} >
      ...
      <Listbox value={font} onChange={(e) => onChange(type, e)}>
        <div className="relative mt-2">
          <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-md bg-gray-900 dark:bg-white text-gray-300 dark:text-gray-900 py-1.5 pr-2 pl-3 text-left outline-1 -outline-offset-1 outline-gray-300 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-600 text-[12px] hover:cursor-pointer">
            <span
              className={`col-start-1 row-start-1 flex items-center gap-3 pr-6 ${font.value}`}
            >
