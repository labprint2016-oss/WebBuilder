  ...
  const getMode = ()=>{
    ...
    const handleDropElement = (e)=>{
      ...
      if(draggingRef.current) {
        cancelAnimationFrame(draggingRef.current);
        draggingRef.current = null;
      }
      ...
      if(layouts.length === 0)return
       ...
       if(!Number.isInteger(conI) || !Number.isInteger(colI) || !Number.isInteger(eleI)) {
        clearDropState();
        return;
      }
      
      if(conI < 0 || conI >= layouts.length) {
        clearDropState();
        return;
      }
      
      if(colI < 0 || colI >= layouts[conI].columns.length) {
        clearDropState();
        return;
      }
