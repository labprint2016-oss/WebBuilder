...
console.log(1);
  ...
  const SortableColumnItem = ({ id, containerId, elementData, children}) => {
    ...
    return (
      ...
      >
        {isSpan ? (
         <div className="grid grid-flow-row grid-rows-12 gap-4" ref={(el)=>{
  ...
  const SortableSpanItem = ({
    ...
    {hover?.[1] === id && hover?.[0] === columnId && !activeID && (
            <div className="z-[1000] absolute pointer-events-auto" onMouseEnter={() => setHover([columnId,id])}>
              <OptionButtonGroup
                element={{ colData: elementData, conID: containerId }}
                clone={cloneColumn}
                id={{ conID: containerId, colID: columnId }}
                remove={deleteColumn}
                offcavanas="Span"
                onUpdate={updateColumn}
                modal={openModal}
  ...
  const SortableElementItem = ({
    ...
    const {
      ...
      id,
      data: { type: "ELEMENT", conID: containerId, colID: columnId ,spnID:spanId?spanId:null},
      animateLayoutChanges: noLayoutAnimWhileSorting,
    });
    const style = {
      transform: CSS.Transform.toString(transform),
  ...
  function OptionButtonGroup({
    ...
    return (
      <div className="flex items-center justify-center absolute -top-px -left-px"  onMouseMove={(e)=>{
        ...
        {(offcavanas === "Column" || offcavanas === "Span") && (
          <button
            ...
            onMouseEnter={() => {onDragAbleCol();disableSpanDrag()}}
            onMouseOver={() => {onDragAbleCol();disableSpanDrag()}}
            onMouseLeave={() => {onDragDisableCol();disableSpanDrag()}}
          >
        ...
        <button className=" bg-gray-900  text-white px-[3px] py-1" type="button" draggable={false}>
          <Trash2
            ...
            onMouseDownCapture={stopAll}
            onTouchStartCapture={stopAll}
            onClick={(e) => {modal({ id: id, funct: remove })}}
          />
        </button>
      </div>
    );
        ...
        <Fade in={open} timeout={200} onExited={()=>setOpen(false)}>
          ...
          >
            ...
            <div
              className={`border-b border-dotted border-gray-500/50 flex-1`}
            ></div>
            <div className="flex justify-center mt-2 mb-5 text-[13px] ">
