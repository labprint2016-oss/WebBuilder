...
const Content = ({
  ...
  const addNewElement = (element,i1,i2,i3) => {
    ...
    setLayout((prev) => {
      ...
      if(Array.isArray(i2)){
        const [colI,spnI] = i2
        newLayouts[i1].columns[colI].spans[spnI].elements.splice(i3, 0, element);
      }else{
  ...
  const dropNewElement = () => {
    ...
    if( !Number.isInteger(dropTargetRef.current?.index?.conI) || ( !Number.isInteger(dropTargetRef.current?.index?.colI) && !Array.isArray(dropTargetRef.current?.index?.colI)) || !Number.isInteger(dropTargetRef.current?.index?.eleI) || typeof dropTargetRef.current.index !== "object" || dropTargetRef.current.type !== "ELEMENT") {
      clearGhost()
      return
    }
  ...
  const setDropForElement = (conID, colID,overCol,overSpan, overEl,mouseX,mouseY,element)=>{
    ...
    if(overSpan){
      ...
      const id = overSpan.getAttribute("id")
      let index = layouts[conI].columns[colI].spans.findIndex(s => s.id === id)
      const eleSpn =  layouts[conI].columns[colI].spans[index]?.elements
      if(!eleSpn) return
      ...
      if(!eleSpn.length){
        return {index:{conI,colI:[colI,index],eleI:0},type:"ELEMENT",isLast:false}
      }
  
      if(!overEl){
        return {index:{conI,colI:[colI,index],eleI:eleSpn.length},type:"ELEMENT",isLast:true}
      }
      ...
      eleI += (mouseY > mid ? 1:0)
      return { index: { conI, colI:[colI,index], eleI },type:"ELEMENT", isLast: eleI === eleSpn.length };
  ...
  const updateHoverFromPoint = (x, y, type, element) => {
    ...
    if (type === "SECTION") {
      ...
      const dropElement = setDropForElement(conID,colID,column,span,EL,x,y,element)
      if(!dropElement){
        set_2_null()
        return
      }
  ...
  const deleteElement = (id) => {
    ...
    setLayout((prev) => {
      ...
      if(Array.isArray(colID)){
        const [cID,sID] = colID
        const idx = newLayouts[IDX].columns.findIndex((c) => c.id === cID);
        const sidx = newLayouts[IDX].columns[idx].spans.findIndex((s)=> s.id === sID)
    ...
    return (
      ...
      >
        ...
        >
            ...
            <OptionButtonPortal className="z-[1000]" onMouseEnter={() => setHover(id)} visible={hover === id && !activeID}>
              <OptionButtonGroup
  ...
  const SortableSpanItem = ({
    ...
    const {size} = layouts[IDX].columns[idx].spans[sidx]
    ...
    const eleLength = sidx > -1 ? spans[sidx].elements.length : 0

    


    const setHeight = ()=>{
       if(eleLength > 0 || (dropTargetRef.current.index?.colI?.[0] === idx && dropTargetRef.current.index?.colI?.[1] === sidx && dropTargetRef.current.index?.conI === IDX && hugeElementType.includes(preview?.type) )){
        return "min-h-[40px]"
       }else{
        return "h-[92px]"
       }
  ...
  const SortableElementItem = ({
    ...
    const {
      ...
      id,
      data: { type: "ELEMENT", conID: containerId, colID: columnId ,spnID:spanId},
      animateLayoutChanges: noLayoutAnimWhileSorting,
    });
    const style = {
      transform: CSS.Transform.toString(transform),
    ...
    return (
      <Box
        ...
        onClick={(e) => {
          ...
          setDeleteID({ conID: containerId, colID: spanId?[columnId,spanId]:columnId, eleID: id });
        }}
...
<List dense sx={{ width: '100%',py:0,my:0.5}}>
  ...
  const change_element_position = (
    ...
    if(Number.isInteger(oldSpanIndex) && Number.isInteger(newSpanIndex)){
      const oldElements = newLayouts[containerIndex].columns[columnIndex].spans[oldSpanIndex].elements
...
<List dense sx={{ width: '100%',py:0,my:0.5}}>
  ...
  const findElementIndexForDND = (conI,colI,conID,colID,eleID,active,spnI=null)=>{
  ...
  const drag = ({ active }) => {
    ...
    if (id.startsWith("Sec-")) {
      ...
      setActiveID({ conID: current.conID, colID: id ,spnID:null});
  ...
  const during = ({ active, over }) => {
    ...
    const types = ["COLUMN", "ELEMENT","SPAN"];
    if (
      ...
      if (
        ...
        if (oldColumnID === newColumnID && oldContainerID === newContainerID) {
          ...
          let oldIndex,newIndex,sidx1,sidx2
          if(active.data.current.spnID && over.data.current.spnID){
            ...
            const target = layouts[IDX].columns[idx].spans[sidx2].elements
            if(target.length === 0){
              change_element_position(oldIndex, 0, IDX, idx,sidx1,sidx2);
              return
            }else{
          ...
          change_element_position(oldIndex, newIndex, IDX, idx,sidx1,sidx2);
        } else if (
          ...
          const targetLength = sidx2 >= 0? layouts[IDX].columns[idx2].spans[sidx2].elements.length : layouts[IDX].columns[idx2].elements.length
          ...
          if (targetLength === 0 ) {
            ...
            change_element_position_new_column(oldIndex, 0, IDX, idx1, idx2,sidx1,sidx2); return
          } else {

            newIndex = findElementIndexForDND(IDX,idx2,newContainerID,newColumnID,over.id,active,sidx2)
          ...
          if (targetLength === 0) {
            ...
            newIndex = findElementIndexForDND(IDX2,idx2,newContainerID,newColumnID,over.id,active,sidx2)
            if (newIndex === -1) return;
                            ...
                            return (
                              ...
                              >
                                {isSpan ? (<SortableContext items={spanID} strategy={verticalListSortingStrategy} disabled={disableSpnDrag}>
                                    ...
                                    {spans.map((s,o)=>{
                                      const {elements:eleSpn,id:sid} = s
                                      const eleSpnID = eleSpn.map(e => e.id) || ["ele-spn-null"]
                                      return (<SortableSpanItem key={sid} id={sid} columnId={id} containerId={ID} elementData={col}>
                                            ...
                                            {eleSpn.map((e,_)=>{
                                              ...
                                              return(
                                               <React.Fragment key={e.id}>
                                                {preview &&  !dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI[0] === i && dropTargetRef.current.index?.colI[1] === o && dropTargetRef.current.index?.eleI === _ && ( <div
                                                    ref={ghostRef}
                                                    className=" opacity-70 "
                                                    key="ghost-ele"
                                          ...
                                          {preview &&  dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI[0] === i && dropTargetRef.current.index?.colI[1] === o && _ === eleSpn.length-1 && ( <div
                                                    ref={ghostRef}
                                                    className=" opacity-70 "
                                                    key="ghost-ele"
                                              ...
                                              {preview &&  !dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI[0] === i && dropTargetRef.current.index?.colI[1] === o ? ( <div
                                                    ref={ghostRef}
                                                    className=" opacity-70 "
                                                    key="ghost-ele"
              ...
              ((typeof activeID === "string" && (
                ...
                (typeof activeID === "object" && !activeID.eleID &&  !activeID.spnID && (
                  ...
                  <SpanPreview elementData={activeItem}>
                      {activeItem?.spans[activeItem.spans.findIndex(s => s.id === activeID.spnID)].elements.length === 0 ? (
                      <>{activeID.spnID}</>
                    ) : (
                      <div>
                        {activeItem.elements.map((ele) => (
  ...
  function OptionButtonGroup({
    ...
    return (
      <div className="flex items-center justify-center absolute -top-px -left-px" data-keep-hover="1" onMouseMove={(e)=>{
        scheduleDND(e)
      }} >
        {(offcavanas === "Column" || (offcavanas === "Span" && spnIndex === 0)) && (
          <button
            ...
            onMouseEnter={() => {onDragAbleCol()}}
            onMouseOver={() => {onDragAbleCol()}}
            onMouseLeave={() => {onDragDisableCol()}}
          >
        ...
        type="button"
          className=" bg-gray-900  text-white  px-[3px] py-1"
          draggable={false}
          onClick={(e) => {openOffcavanas(offcavanas === "Span"?"Column":offcavanas, element, onUpdate)}}
        >
  ...
  function OptionButtonPortal({visible,children}){
    if(!visible) return
      return (
        <>{children}</>
      )
