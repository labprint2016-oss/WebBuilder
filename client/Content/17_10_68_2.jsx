...
const Content = ({
  ...
  useEffect(()=>{
    const current = draggingElement?.();
      if(!current){
        setPreview(null)
        return
      }
      const element = lodash.cloneDeep(current)
      if(element.container && element.columns){
        element.container.id += "preview";
        for (let i = 0; i < 3; i++) {
          element.columns[i].id += `${"preview"}-${i}`;
        }
        setPreview(element)
      }else{
        setPreview(element)
  ...
  const clearGhost = () => { 
    ...
    if(btnGroupRef.current) {
      cancelAnimationFrame(btnGroupRef.current);
      btnGroupRef.current = null;
    }
    if(dndRef.current) {
      cancelAnimationFrame(dndRef.current);
      dndRef.current = null;
    }
  };
  ...
  useEffect(() => {
    return () => {
      ...
      if(btnGroupRef.current) cancelAnimationFrame(btnGroupRef.current);
      if(dndRef.current) cancelAnimationFrame(dndRef.current);
    };
  }, []);
    ...
    return (
      ...
      >
        <div
          ...
          className={`column-area border-[1px]  border-dashed border-gray-600 flex ${
            elementData.elements.length > 0 || (dropTarget?.index?.colI === idx && dropTarget?.index?.conI === IDX && hugeElementType.includes(preview?.type))?"h-auto":"h-[200px]"
          } justify-center items-center text-center relative p-1`}
          ref={(el)=>{
...
<List dense sx={{ width: '100%',py:0,my:0.5}}>
  ...
  const ContainerPreview = ({ element, id, children }) => {
    if(preview && !preview.container) return <></>
                      ...
                      {preview &&
                        ...
                        dropTarget?.index === I && (
                          <div
                            ...
                            key={`ghost-sec-${dropTarget?.index}`}
                                              ...
                                              !dropTarget?.isLast && (
                                                <>
                                                  <div
                                                   ref={setGhostRef}
                                                    className=" opacity-70 "
                                                    key={`ghost-ele-${dropTarget?.index?.conI}-${dropTarget?.index?.colI}-${dropTarget?.index?.eleI}`}
