...
const Content = ({
  ...
  const updateDND = (x, y) => {
    ...
    if(section && !column && !element){
      ...
      if(span){
        ...
        if(miniSpan){
            ...
            const spnI = layouts[conI].columns[colI].spans.findIndex(s => s.id === spnIDPath)
            if(spnI === -1) return
            const mspnID = miniSpan.id
            const mspnI = layouts[conI].columns[colI].spans[spnI].miniSpans.findIndex(ms => ms.id === mspnID)
            if(mspnI === -1) return
            isHasElements = layouts[conI].columns[colI].spans[spnI].miniSpans[mspnI].elements.length > 0
        }
        else{
          const spnI = layouts[conI].columns[colI].spans.findIndex(s => s.id === spnID)
          if(spnI === -1) return
          isHasElements = layouts[conI].columns[colI].spans[spnI].elements.length > 0
        }
    ...
    else if((section && column && element && span ) || (section && column && element && span && miniSpan)){
      setDisableConDrag(false)
      setDisableEleDrag(true)
  ...
  const addNewElement = (element,i1,i2,i3,i4=null,i5=null) => {
    setLayout((prev) => {
      ...
      if(i4 !== null){
        if(i5 !== null){
          newLayouts[i1].columns[i2].spans[i4]?.miniSpans[i5].elements.splice(i3, 0, element);
        }else{
  ...
  const dropNewElement = () => {
    ...
    if( !Number.isInteger(dropTargetRef.current?.index?.conI) || !Number.isInteger(dropTargetRef.current?.index?.colI) || !Number.isInteger(dropTargetRef.current?.index?.eleI) || typeof dropTargetRef.current.index !== "object" || dropTargetRef.current.type !== "ELEMENT") {
      clearGhost()
      return
    }
    let spnI=null,mspnI = null
  ...
  const setDropForElement = (conID, colID,overCol,overSpan,overMiniSpan, overEl,mouseX,mouseY,element)=>{
    ...
    if(overSpan){
      ...
      if(overMiniSpan){
        ...
        let spnI = layouts[conI].columns[colI].spans.findIndex(s => s.id === spnID)
        const msID = overMiniSpan.getAttribute("id")
        const mspnI = layouts[conI].columns[colI].spans[spnI].miniSpans.findIndex(ms => ms.id === msID)
        const eleMspn = layouts[conI].columns[colI].spans[spnI]?.miniSpans[mspnI]?.elements
        if(!eleMspn) return
        const rectMspn = overMiniSpan.getBoundingClientRect()
        if(mouseY < rectMspn.top || mouseY > rectMspn.bottom) return
        setPreview(element);
        if(!eleMspn.length){
          return {index:{conI,colI,spnI,mspnI,eleI:0},type:"ELEMENT",isLast:false}
        }
        if(!overEl){
          return {index:{conI,colI,spnI,mspnI,eleI:eleMspn.length},type:"ELEMENT",isLast:true}
        }
        ...
        eleI += (mouseY > mid ? 1:0)
        return { index: { conI, colI,spnI,mspnI, eleI },type:"ELEMENT", isLast: eleI === eleMspn.length };
      }else{
        let spnI = layouts[conI].columns[colI].spans.findIndex(s => s.id === sid)
        ...
        eleI += (mouseY > mid ? 1:0)
        return { index: { conI, colI,spnI, eleI },type:"ELEMENT", isLast: eleI === eleSpn.length };
      }
  ...
  const updateHoverFromPoint = (x, y, type, element) => {
    ...
    if (ghostRef.current) {
      ...
      ? typeof dropTargetRef.current.index === "number"
      : dropTargetRef.current.index && Number.isInteger(dropTargetRef.current.index.conI) &&(Number.isInteger(dropTargetRef.current.index.colI) || Array.isArray(dropTargetRef.current.index.colI)) && Number.isInteger(dropTargetRef.current.index.eleI)
    )
    ...
    if (type === "SECTION") {
      ...
      const dropElement = setDropForElement(conID,colID,column,span,miniSpan,EL,x,y,element)
      if(!dropElement){
        set_2_null()
        return
      }
  ...
  const updateSpan = (data, id, conID,colID) => {
    setLayout((prev) => {
      ...
      const sidx = newLayouts[IDX].columns[idx].spans.findIndex(s => s.id === id)
  ...
  const updateMiniSpan = (data, id, conID,colID,spnID) => {
    setLayout((prev) => {
      ...
      const IDX = newLayouts.findIndex((l) => l.container.id === conID);
      const idx = newLayouts[IDX].columns.findIndex(c => c.id === colID)
      const sidx = newLayouts[IDX].columns[idx].spans.findIndex(s => s.id === spnID)
      const msidx = newLayouts[IDX].columns[idx].spans[sidx].miniSpans.findIndex(ms => ms.id === id)
  ...
  const deleteElement = (id) => {
    ...
    setLayout((prev) => {
      ...
      if(spnID){
        const sidx = newLayouts[IDX].columns[idx].spans.findIndex((s)=> s.id === spnID)
        if(mspnID){
          const msidx = newLayouts[IDX].columns[idx].spans[sidx].miniSpans.findIndex((ms)=>ms.id ===mspnID)
          newElements = newLayouts[IDX].columns[idx].spans[sidx].miniSpans[msidx].elements;
  ...
  const SortableColumnItem = ({ id, containerId, elementData, children}) => {
    ...
    return (
      ...
      >
        {isSpan ? (
         <div className="grid grid-flow-row-dense auto-rows-[minmax(40px,auto)] gap-4" ref={(el)=>{
        ...
        <div
          className={`column-area border-[1px]  border-dashed border-gray-600 flex ${
            elementData.elements.length > 0 || (dropTargetRef.current.index?.colI === idx && dropTargetRef.current.index?.conI === IDX && hugeElementType.includes(preview?.type))?"min-h-[40px]":"h-[200px]"
  ...
  const SortableSpanItem = ({
    ...
    const eleLength = sidx > -1 ? layouts[IDX].columns[idx].spans[sidx].elements.length : 0

    


    const setHeight = ()=>{
       if(eleLength > 0 || (dropTargetRef.current.index?.colI === idx && dropTargetRef.current.index?.spnI === sidx && dropTargetRef.current.index?.conI === IDX && hugeElementType.includes(preview?.type) )){
        return "min-h-[40px]"
       }else{
        return "h-[92px]"
       }
    ...
    return (
      ...
      >
        ...
        {isMiniSpan? (<div className={`col-span-12`}>
        <div className="grid grid-cols-12 gap-4 w-full">
          {children}
        </div>

        </div> ):( <div
          className={`border-[1px] ${setHeight()} border-dashed border-gray-600 flex-1 justify-center items-center text-center relative p-1 col-span-${size}`}
          data-drop="SPAN"
          id={id}
          onDragOver={(e) => {
            handleDuring(e);
    ...
    {hover === id && !activeID && (
            <div className="z-[1000] absolute pointer-events-auto" onMouseEnter={() => setHover(id)}>
              <OptionButtonGroup
                element={{ spnData: elementData, conID: containerId ,colID:columnId}}
                clone={cloneColumn}
                cloneSpn={cloneSpan}
  ...
  const SortableMiniSpanItem = ({id,elementData,containerId,columnId,spanId,children})=>{
    const hugeElementType = ["img", "yt", "gly"];
    const IDX = layouts.findIndex((l) => l.container.id === containerId);
    const idx = layouts[IDX].columns.findIndex((c) => c.id === columnId);
    const sidx = layouts[IDX].columns[idx].spans.findIndex((s) => s.id === spanId);
    const msidx = layouts[IDX].columns[idx].spans[sidx].miniSpans.findIndex((ms) => ms.id === id);
   ...
   const brColor =
     typeof borderColor === "string"
       ...
       : theme[borderColor.type][borderColor.index] +
         ...
         const eleLength = sidx > -1 ? layouts[IDX].columns[idx].spans[sidx].miniSpans[msidx].elements.length : 0

    


    const setHeight = ()=>{
       if(eleLength > 0 || (dropTargetRef.current.index?.colI === idx && dropTargetRef.current.index?.spnI === sidx && dropTargetRef.current.index?.mspnI === msidx && dropTargetRef.current.index?.conI === IDX && hugeElementType.includes(preview?.type) )){
        return "min-h-[40px]"
       }else{
        return "h-[92px]"
       }
    ...
    return (
      <div className={`col-span-${size}`} ref={setNodeRef} {...attributes} style={style} data-drop="MINI-SPAN" id={`${containerId}/${columnId}/${spanId}/${id}`}>
          <div
        ...
        >
          {hover === id && !activeID && (
            <div className="z-[1000] absolute pointer-events-auto" onMouseEnter={() => setHover(id)}>
              <OptionButtonGroup
                element={{ mspnData: elementData, conID: containerId ,colID:columnId,spnID:spanId}}
                clone={cloneColumn}
                cloneSpn={cloneSpan}
                cloneMspn={cloneMiniSpan}
                id={{ conID: containerId, colID: columnId ,spnID:spanId,mspnID:id}}
                remove={deleteColumn}
                removeSpn={deleteSpan}
  ...
  const SortableElementItem = ({
    ...
    const {
      ...
      id,
      data: { type: "ELEMENT", conID: containerId, colID: columnId ,spnID:spanId,mspnID:miniSpanId},
      animateLayoutChanges: noLayoutAnimWhileSorting,
    ...
    return (
      <Box
        ...
        onClick={(e) => {
          ...
          setDeleteID({ conID: containerId, colID: columnId,spnID:spanId,mspnID:miniSpanId, eleID: id });
        }}
...
<List dense sx={{ width: '100%',py:0,my:0.5}}>
  ...
  const change_span_position = (oldIndex,newIndex,containerIndex,columnIndex) =>{
    const newLayouts = lodash.cloneDeep(layouts)
    const newSpans = newLayouts[containerIndex].columns[columnIndex].spans
    const [span] = newSpans.splice(oldIndex,1)
    newSpans.splice(newIndex,0,span)
    setLayout((prev) => {
      return newLayouts
    })
  }

  const change_mini_span_position = (oldIndex,newIndex,containerIndex,columnIndex,spanIndex) =>{
    const newLayouts = lodash.cloneDeep(layouts)
    const newMiniSpans = newLayouts[containerIndex].columns[columnIndex].spans[spanIndex].miniSpans
    const [miniSpan] = newMiniSpans.splice(oldIndex,1)
    newMiniSpans.splice(newIndex,0,miniSpan)
    setLayout((prev) => {
      return newLayouts
    })
  }
  ...
  const change_element_position = (
    ...
    if(Number.isInteger(oldSpanIndex) && Number.isInteger(newSpanIndex)){
      ...
      if(Number.isInteger(oldMiniSpanIndex)){
        oldElements = newLayouts[containerIndex].columns[columnIndex].spans[oldSpanIndex].miniSpans[oldMiniSpanIndex].elements
      }else{
        oldElements = newLayouts[containerIndex].columns[columnIndex].spans[oldSpanIndex].elements
      }

      if(Number.isInteger(newMiniSpanIndex)){
        newElements = newLayouts[containerIndex].columns[columnIndex].spans[newSpanIndex].miniSpans[newMiniSpanIndex].elements;
      }else{
        newElements = newLayouts[containerIndex].columns[columnIndex].spans[newSpanIndex].elements;
      }
  ...
  const change_element_position_new_column = (
    ...
    if(Number.isInteger(oldSpanIndex)){
      if(Number.isInteger(oldMiniSpanIndex)){
        oldElements =
      newLayouts[containerIndex].columns[oldColumnIndex].spans[oldSpanIndex].miniSpans[oldMiniSpanIndex].elements;
      }else{
        oldElements =
      newLayouts[containerIndex].columns[oldColumnIndex].spans[oldSpanIndex].elements;
      }
    }else{
      oldElements =
    ...
    if(Number.isInteger(newSpanIndex)){
      if(Number.isInteger(newMiniSpanIndex)){
        newElements =
        newLayouts[containerIndex].columns[newColumnIndex].spans[newSpanIndex].miniSpans[newMiniSpanIndex].elements;
      }else{
        newElements =
        newLayouts[containerIndex].columns[newColumnIndex].spans[newSpanIndex].elements;
      }
  ...
  const change_element_position_new_container = (
    ...
    let oldElements,newElements
      ...
      if(Number.isInteger(oldSpanIndex)){
        if(Number.isInteger(oldMiniSpanIndex)){
          oldElements =
        newLayouts[oldContainerIndex].columns[oldColumnIndex].spans[oldSpanIndex].miniSpans[oldMiniSpanIndex].elements;
        }else{
          oldElements =
        newLayouts[oldContainerIndex].columns[oldColumnIndex].spans[oldSpanIndex].elements;
        }
      ...
      if(Number.isInteger(newSpanIndex)){
        if(Number.isInteger(newMiniSpanIndex)){
          newElements =
          newLayouts[newContainerIndex].columns[newColumnIndex].spans[newSpanIndex].miniSpans[newMiniSpanIndex].elements;
        }else{
          newElements =
          newLayouts[newContainerIndex].columns[newColumnIndex].spans[newSpanIndex].elements;
        }
  ...
  const SpanPreview = ({
    ...
    return (
      ...
      >
        <div
          className={`border-[1px] h-[92px] border-dashed border-gray-600 flex-1 justify-center items-center text-center relative p-1`}
          data-drop="SPAN"
...
<List dense sx={{ width: '100%',py:0,my:0.5}}>
  ...
  const ElementPreviewForDrag_Drop = ({ element }) => {
    ...
    return (
      <div
      className="bg-gray-50 dark:bg-black/50 w-[95.5px] h-[70px] rounded-md text-center px-3 py-2 cursor-grab active:cursor-grabbing"
  ...
  const findElementIndexForDND = (conI,colI,conID,colID,eleID,active,spnI=null,mspnI=null)=>{

    let index

    if(Number.isInteger(spnI)){
      if(Number.isInteger(mspnI)){
        index = layouts[conI].columns[colI].spans[spnI].miniSpans[mspnI].elements.findIndex(e => e.id === eleID)
      }else{  
        index = layouts[conI].columns[colI].spans[spnI].elements.findIndex(e => e.id === eleID)
      }
    }else{
      index = layouts[conI].columns[colI].elements.findIndex(e => e.id === eleID)
    }
  ...
  const drag = ({ active }) => {
    ...
    if (id.startsWith("Sec-")) {
      ...
      setActiveID({ conID: current.conID, colID: current.colID,spnID:current.spnID,mspnID:id });
    }
      else {
      section = layouts.find((l) => l.container.id === current.conID);
      ...
      setActiveID({ conID: current.conID, colID: current.colID, eleID: id ,spnID:current.spnID?current.spnID:null});
    }
  };
  ...
  const during = ({ active, over }) => {
    ...
    if (
      ...
      if (
        ...
        if (oldColumnID === newColumnID && oldContainerID === newContainerID) {
          ...
          if(active.data.current.spnID && over.data.current.spnID){
            sidx1 = layouts[IDX].columns[idx].spans.findIndex(s => s.id === active.data.current.spnID)
            sidx2 = layouts[IDX].columns[idx].spans.findIndex(s => s.id === over.data.current.spnID)
            if(active.data.current.mspnID){
              msidx1 = layouts[IDX].columns[idx].spans[sidx1].miniSpans.findIndex(ms => ms.id === active.data.current.mspnID)
              oldIndex = layouts[IDX].columns[idx].spans[sidx1].miniSpans[msidx1].elements.findIndex(
                (e) => e.id === active.id
              );
            }else{
              oldIndex = layouts[IDX].columns[idx].spans[sidx1].elements.findIndex(
                (e) => e.id === active.id
              );
            }
            if (oldIndex === -1 ) return;
            if(over.data.current.mspnID){
              msidx2 = layouts[IDX].columns[idx].spans[sidx2].miniSpans.findIndex(ms => ms.id === over.data.current.mspnID)
              target = layouts[IDX].columns[idx].spans[sidx2].miniSpans[msidx2].elements
            }else{
              target = layouts[IDX].columns[idx].spans[sidx2].elements
            } 
           
            if(target.length === 0){
              change_element_position(oldIndex, 0, IDX, idx,sidx1,sidx2,msidx1,msidx2);
              return
            }else{
              newIndex = target.findIndex(
                (e) => e.id === over.id
              );
            }
          }else{
          ...
          change_element_position(oldIndex, newIndex, IDX, idx,sidx1,sidx2,msidx1,msidx2);
        } else if (
          ...
          if(active.data.current.spnID){
            ...
            sidx1 = layouts[IDX].columns[idx1].spans.findIndex(s => s.id === spnID)
            if(active.data.current.mspnID){
              const {mspnID} = active.data.current
              msidx1 = layouts[IDX].columns[idx1].spans[sidx1].miniSpans.findIndex(ms => ms.id === mspnID)
              oldIndex = layouts[IDX].columns[idx1].spans[sidx1].miniSpans[msidx1].elements.findIndex(
                (e) => e.id === active.id
              );
            }else{
              oldIndex = layouts[IDX].columns[idx1].spans[sidx1].elements.findIndex(
                (e) => e.id === active.id
              );
            }
          }else{
          ...
          if(over.data.current.spnID){
            ...
            sidx2 = layouts[IDX].columns[idx2].spans.findIndex(s => s.id === spnID)
            if(over.data.current.mspnID){
              const {mspnID} = over.data.current
              msidx2 = layouts[IDX].columns[idx2].spans[sidx2].miniSpans.findIndex(ms => ms.id === mspnID)
            }
          }


          const targetLength = msidx2 >= 0? layouts[IDX].columns[idx2].spans[sidx2].miniSpans[msidx2].elements.length: sidx2 >= 0? layouts[IDX].columns[idx2].spans[sidx2].elements.length :layouts[IDX].columns[idx2].elements.length
          ...
          if (targetLength === 0 ) {
            ...
            change_element_position_new_column(oldIndex, 0, IDX, idx1, idx2,sidx1,sidx2,msidx1,msidx2); return
          } else {

            newIndex = findElementIndexForDND(IDX,idx2,newContainerID,newColumnID,over.id,active,sidx2,msidx2)
            ...
            change_element_position_new_column(
              ...
              IDX,
              idx1,
              idx2,sidx1,sidx2,msidx1,msidx2
            );
          }
          ...
          if(active.data.current.spnID){
            const {spnID} = active.data.current
            sidx1 = layouts[IDX1].columns[idx1].spans.findIndex(s => s.id === spnID)
            if(active.data.current.mspnID){
              const {mspnID} = active.data.current
              msidx1 = layouts[IDX1].columns[idx1].spans[sidx1].miniSpans.findIndex(ms => ms.id === mspnID)
              oldIndex = layouts[IDX1].columns[idx1].spans[sidx1].miniSpans[sidx1].elements.findIndex(
                (e) => e.id === active.id
              );
            }else{
              oldIndex = layouts[IDX1].columns[idx1].spans[sidx1].elements.findIndex(
                (e) => e.id === active.id
              );
            }
          ...
          if(over.data.current.spnID){
            const {spnID} = over.data.current
            sidx2 = layouts[IDX2].columns[idx2].spans.findIndex(s => s.id === spnID)
            if(over.data.current.mspnID){
              const mspnID = over.data.current.mspnID
              msidx2 = layouts[IDX2].columns[idx2].spans[sidx2].miniSpans.findIndex(ms => ms.id === mspnID)
            }
          }
          ...
          if(Number.isInteger(sidx2)){
            if(Number.isInteger(msidx2)){
              targetLength = layouts[IDX2].columns[idx2].spans[sidx2].miniSpans[msidx2].elements.length
            }else{
              targetLength = layouts[IDX2].columns[idx2].spans[sidx2].elements.length
            }
          ...
          if (targetLength === 0) {
            ...
            newIndex = findElementIndexForDND(IDX2,idx2,newContainerID,newColumnID,over.id,active,sidx2,msidx2)
            if (newIndex === -1) return;
      ...
      }else if(over.data.current.type === "SPAN" &&
      active.data.current.type === "SPAN"){
        const IDX = layouts.findIndex(l => l.container.id === oldContainerID)
        if(IDX ===-1) return
        const columnID = active.data.current.colID ?? over.data.current.colID
        const idx = layouts[IDX].columns.findIndex(c => c.id === columnID)
        if(idx === -1) return
        const oldIndex = layouts[IDX].columns[idx].spans.findIndex(s => s.id === active.id)
        if(oldIndex === -1)return
        const newIndex = layouts[IDX].columns[idx].spans.findIndex(s => s.id === over.id)
        if(newIndex === -1)return
        change_span_position(oldIndex,newIndex,IDX,idx)
        
        
      }else if(over.data.current.type === "MINI-SPAN" &&
      active.data.current.type === "MINI-SPAN"){
        const IDX = layouts.findIndex(l => l.container.id === oldContainerID)
        if(IDX ===-1) return
        const columnID = active.data.current.colID ?? over.data.current.colID
        const idx = layouts[IDX].columns.findIndex(c => c.id === columnID)
        if(idx === -1) return
        const spnID = active.data.current.spnID ?? over.data.current.spnID
        const sidx = layouts[IDX].columns[idx].spans.findIndex(s => s.id === spnID)
        const oldIndex = layouts[IDX].columns[idx].spans[sidx].miniSpans.findIndex(s => s.id === active.id)
        if(oldIndex === -1)return
        const newIndex = layouts[IDX].columns[idx].spans[sidx].miniSpans.findIndex(s => s.id === over.id)
        if(newIndex === -1)return
        change_mini_span_position(oldIndex,newIndex,IDX,idx,sidx)
      }
    ;
  };
                          ...
                          {columns.map((col, i) => {
                            ...
                            return (
                              <SortableColumnItem
                                ...
                                elementData={col}
                              >
                                {isSpan ? (<SortableContext items={spanID} strategy={verticalListSortingStrategy} disabled={disableSpnDrag}>
                                
                                    {spans.map((s,o)=>{
                                      const {elements:eleSpn,id:sid,isMiniSpan,miniSpans} = s
                                      const eleSpnID = eleSpn.map(e => e.id) || ["ele-spn-null"]
                                      const minSpnID = miniSpans.map(ms => ms.id)
                                      return (<SortableSpanItem key={sid} id={sid} columnId={id} containerId={ID} elementData={s}>
                                        {isMiniSpan ? (<SortableContext items={minSpnID} strategy={rectSortingStrategy} disabled={disableMspnDrag}>
                                          {miniSpans.map((ms,_)=>{
                                            const {elements:eleMspn,id:msid} = ms
                                            const eleMspnID = eleMspn.map(e => e.id) ?? ["ele-mspn-null"]
                                            return (
                                              <SortableMiniSpanItem containerId={ID} columnId={id} spanId={sid} id={msid} key={msid} elementData={ms}>
                                                 <SortableContext items={eleMspnID} strategy={verticalListSortingStrategy} disabled={disableEleDrag}>
                                                 {eleMspn.length > 0 ?(<>
{eleMspn.map((e,__)=>{
  
  return(
   <React.Fragment key={e.id}>
    {preview &&  !dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI === i && dropTargetRef.current.index?.spnI === o && dropTargetRef.current.index?.mspnI === _ && dropTargetRef.current.index?.eleI === __ && ( <div
        ref={ghostRef}
        className=" opacity-70 "
        key="ghost-ele"
...
{preview &&  dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI === i && dropTargetRef.current.index?.spnI === o && dropTargetRef.current.index?.mspnI === _ &&__ === eleMspn.length-1 && ( <div
        ref={ghostRef}
        className=" opacity-70 "
        key="ghost-ele"
  ...
  {preview &&  !dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI === i && dropTargetRef.current.index?.spnI === o && dropTargetRef.current.index?.mspnI === _ ? ( <div
        ref={ghostRef}
        className=" opacity-70 "
        key="ghost-ele"
...
<SortableContext items={eleSpnID} strategy={verticalListSortingStrategy} disabled={disableEleDrag}>
...
{eleSpn.map((e,_)=>{
  ...
  return(
   <React.Fragment key={e.id}>
    {preview &&  !dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI === i && dropTargetRef.current.index?.spnI === o && dropTargetRef.current.index?.eleI === _ && ( <div
        ref={ghostRef}
        className=" opacity-70 "
        key="ghost-ele"
...
{preview &&  dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI === i && dropTargetRef.current.index?.spnI === o && _ === eleSpn.length-1 && ( <div
        ref={ghostRef}
        className=" opacity-70 "
        key="ghost-ele"
  ...
  {preview &&  !dropTargetRef.current.isLast && dropTargetRef.current.type === "ELEMENT" && dropTargetRef.current.index?.conI === I && dropTargetRef.current.index?.colI === i && dropTargetRef.current.index?.spnI === o ? ( <div
        ref={ghostRef}
        className=" opacity-70 "
        key="ghost-ele"
...
{sid}
                                ...
                                >
                                  {elements.length > 0 ? (
                                    ...
                                    <>
                                      {preview &&
                                      dropTargetRef.current.index?.conI === I &&
          ...
          >
            {activeID &&
              ...
              ((typeof activeID === "string" && (
                ...
                )) || (typeof activeID === "object" && activeID.spnID && !activeID.eleID && !activeID.mspnID && (
                ...
                )) || (typeof activeID === "object" && activeID.mspnID &&  !activeID.eleID && (
                  <MiniSpanPreview element={activeItem}>{activeID.mspnID}</MiniSpanPreview>
                )) )}
          </DragOverlay>
        </div>
      </DndContext>
  ...
  function OptionButtonGroup({
    ...
    const getID = (forSpan = false)=>{
      if(typeof id === "string" || !id.spnID || forSpan){
        return id
      }else{
        const {conID,colID} = id
        return {conID,colID}
    ...
    return (
      <div className="flex items-center justify-center absolute -top-px -left-px" onMouseMove={(e)=>{
        scheduleDND(e)
      }} data-drop="BTN">
        {(offcavanas === "Column" || (offcavanas === "Span" && spnIndex === 0) || (offcavanas === "Mini Span" && mspnIndex === 0 && spnIndex === 0)) && (
          <button
            className="bg-gray-900  text-white px-[3px] py-1"
            ref={offcavanas === "Column" && dndHandleCol ? dndHandleCol.setActivatorNodeRef : null}
            {...(offcavanas === "Column" && dndHandleCol ? dndHandleCol.listeners : {})}
            onMouseEnter={() => {onDragAbleCol();if(onDragDisableSpn)onDragDisableSpn()}}
            onMouseOver={() => {onDragAbleCol();if(onDragDisableSpn)onDragDisableSpn()}}
            onMouseLeave={() => {onDragDisableCol();if(onDragDisableSpn)onDragDisableSpn()}}
          >
            <Move className="size-4 m-[5px]" />
          </button>
        )}
         {(offcavanas === "Span" || (offcavanas === "Mini Span" && mspnIndex === 0))&& (
          <button

          type="button"
            className="bg-white  text-gray-900 px-[3px] py-1"
            ref={dndHandleSpn ? dndHandleSpn.setActivatorNodeRef : null}
            {...(dndHandleSpn ? dndHandleSpn.listeners : {})}
            onMouseEnter={() => {onDragAbleSpn()}}
            onMouseOver={() => {onDragAbleSpn()}}
            onMouseLeave={() => {onDragDisableSpn()}}
          >
            <Move className="size-4 m-[5px]" />
          </button>
        )}
         {offcavanas === "Mini Span"&& (
          ...
          type="button"
            className="bg-white  text-gray-900 px-[3px] py-1"
            ref={dndHandleMspn ? dndHandleMspn.setActivatorNodeRef : null}
          {...(dndHandleMspn ? dndHandleMspn.listeners : {})}
            onMouseEnter={() => {onDragAbleMspn();onDragDisableSpn()}}
            onMouseOver={() => {onDragAbleMspn();onDragDisableSpn()}}
            onMouseLeave={() => {onDragDisableMspn();onDragDisableSpn()}}
          >
            <Move className="size-4 m-[5px]" />
          </button>
        )}
        ...
        type="button"
          className=" bg-gray-900  text-white  px-[3px] py-1"
          draggable={false}
          onClick={(e) => {openOffcavanas(offcavanas, element, onUpdate)}}
        >
          <Settings className="size-4 m-[5px]" />
        </button>
        {(offcavanas === "Column" || (offcavanas === "Span" && spnIndex === 0) || offcavanas === "Container") && (
            <button
            type="button"
              className=" bg-gray-900   text-white px-[3px] py-1"
              draggable={false}
              onClick={(e) => {clone(getID())}}
            >
        ...
        {(offcavanas === "Span" || (offcavanas === "Mini Span" && spnIndex === 0 && mspnIndex === 0))&& (
           <button
           type="button"
             className=" bg-white  text-gray-900 px-[3px] py-1"
             draggable={false}
             onClick={(e) => {cloneSpn(getID(true))}}
           >
             <Copy className="size-4 m-[5px]" />
           </button>
        )}
...
{offcavanas === "Mini Span" && (
           ...
           type="button"
             className=" bg-white  text-gray-900 px-[3px] py-1"
             draggable={false}
             onClick={(e) => {cloneMspn(getID(true))}}
           >
             <Copy className="size-4 m-[5px]" />
           </button>
        )}
        {(offcavanas === "Column" || (offcavanas === "Span" && spnIndex === 0)) && (
          <>
            <button
            type="button"
            draggable={false}
              className=" bg-gray-900  text-white px-[3px] py-1"
              onClick={(e) => {changeSizeColumn(getID(), "-")}}
            >
         ...
         {(offcavanas === "Span" || (offcavanas === "Mini Span" && mspnIndex === 0 && spnIndex === 0))  && (
          <>
            <button
            type="button"
            draggable={false}
              className=" bg-white  text-gray-900 px-[3px] py-1"
              onClick={(e) => {changeSizeSpan(getID(true), "-")}}
            >
              <Minus className="size-4 m-[5px]" />
            </button>
            <button
        ...
        {offcavanas === "Mini Span" && (
           ...
           draggable={false}
             className=" bg-white  text-gray-900 px-[3px] py-1"
             onClick={(e) => {changeSizeMiniSpan(getID(true), "-")}}
           >
             <Minus className="size-4 m-[5px]" />
           </button>
           <button
          ...
          <button className=" bg-gray-900  text-white px-[3px] py-1" type="button" draggable={false}>
          <Trash2
            className="size-4 m-[5px]"
            onClick={(e) => {modal({ id: getID(), funct: remove })}}
          />
        </button>
        )}
        
        {(offcavanas === "Span" || (offcavanas === "Mini Span" && mspnIndex === 0 && spnIndex === 0))&& (
           <button className=" bg-white  text-gray-900 px-[3px] py-1" type="button" draggable={false}>
           <Trash2
             className="size-4 m-[5px]"
             onClick={(e) => {modal({ id: getID(true), funct: removeSpn })}}
           />
         </button>
        )}
          ...
          {offcavanas === "Mini Span"&& (
           ...
           <Trash2
             className="size-4 m-[5px]"
             onClick={(e) => {modal({ id: getID(true), funct: removeMspn })}}
           />
         </button>
        )}
      </div>
        ...
        <Fade in={open} timeout={200} onExited={()=>setOpen(false)}>
          ...
          >
            <div className="flex justify-between px-4 pt-3 pb-1">
              ...
              <div>
                <a onClick={(e)=>setOpen(false)} style={{ cursor: "pointer" }}>
                  X
                </a>
              </div>
            </div>
            ...
            คอลัมน์ประเภทนี้ ไม่สามารถปรับขนาดของคอลัมน์ <span className="text-red-600 dark:text-emerald-300">"ให้แคบกว่านี้ได้"</span>
            </div>
