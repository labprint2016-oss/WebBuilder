...
const Content = ({
  ...
  const sameRef = useRef({ index: null, type: null, isLast: false }); // เก็บค่า index ประเภท และใช่ตำแหน่งสุดท้ายไหมของปัจจุบัน เพื่อป้องกันการคำนวณค่าเดิมซ้ำ
  ...
  const updateHoverPosition = (x, y) => {
    ...
    if (!section && !column && !element && !span) {
      setHover(null);
      return;
    }

    if(span && column){
      let [_,__,id] = span.getAttribute("id").split("/")
      if(id){
        setHover(_);
        return "col"
      }
  ...
  const updateDND = (x, y) => {
    ...
    if (section && !column && !element) {
      ...
      if(span){
        ...
        const spanI = layouts[conI].columns[colI].spans.findIndex(s => s.id === spanID)
        isHasElements = layouts[conI].columns[colI].spans[spanI].elements.length > 0
      }else{
  ...
  const addNewElement = (element, i1, i2, i3) => {
    setLayout((prev) => {
      ...
      if (typeof i2 === "object") {
        const [i2_1, i2_2] = i2;
        newLayouts[i1].columns[i2_1].spans[i2_2].elements.splice(
          i3,
          0,
          element
        );
  ...
  const setSame = (i, t, b = false) => {
    sameRef.current = { index: i, type: t, isLast: b };
  };
  ...
  const findSpan = (x, y) => {
    const el = document.elementFromPoint(x, y);
    const nearestSpan = el?.closest(`[data-drop="SPAN"][id*='/']`);
    return nearestSpan ?? null;
  };
  ...
  const setSpanID = (spans, conID, colID) => {
    const id = colID.replace("Col-", "");
    if (spans[0].id !== "Span-") return;
    spans.map((s, i) => {
      if (s.id === "Span-") {
        s.id += `${id}-${i}`;
      }
    ...
    const newLayouts = lodash.cloneDeep(layouts);
    newLayouts[IDX].columns[idx].spans = spans;
    setLayout((prev) => {
      return newLayouts;
    });
  };
  ...
  const setDropForElement = (
    ...
    setPreview(element);
    if (overSpn) {
      const [_1, _2, spanID] = overSpn.getAttribute("id").split("/");
      let index = layouts[conI].columns[colI].spans.findIndex(
        (s) => s.id === spanID
      );
      const elementsForSpan = layouts[conI].columns[colI].spans[index].elements;
      if (!elementsForSpan.length || !elementsForSpan.length === 0) {
        return {
          index: { conI, colI: [colI, index], eleI: 0 },
          type: "ELEMENT",
          isLast: false,
        };
      }
      if (!overEl) {
        return {
          index: { conI, colI: [colI, index], eleI: elementsForSpan.length },
          type: "ELEMENT",
          isLast: true,
        };
      }
      ...
      const [_, __, eleID] = overEl.getAttribute("id").split("/");

      let eleI = elementsForSpan.findIndex((e) => e.id === eleID);
      if (index === -1) {
        return {
          index: { conI, colI: [colI, index], eleI: elementsForSpan.length },
          type: "ELEMENT",
      ...
      eleI += mouseY > mid ? 1 : 0;
      return {
        index: { conI, colI: [colI, index], eleI },
        type: "ELEMENT",
        isLast: eleI === elementsForSpan.length,
      };
    }
  ...
  const cloneColumn = (id) => {
    ...
    newColumn.spans.map((s) => {
      const newID = newColumn.id.replace("Col-", "");
      s.id = `Span-${newID}-${latestSpanID++}`;
      s.elements.map((e) => {
        e.id =
          e.id.split("-")[0] +
    ...
    newColumn.latestSpanID = latestSpanID;
  ...
  const deleteElement = (id) => {
    ...
    setLayout((prev) => {
      ...
      if(Array.isArray(colID)){
        idx = newLayouts[IDX].columns.findIndex((c) => c.id === colID[0]);
        const sidx = newLayouts[IDX].columns[idx].spans.findIndex((s) => s.id === colID[1]);
        const newElements = newLayouts[IDX].columns[idx].spans[sidx].elements;
        const i = newElements.findIndex((e) => e.id === eleID);
        newElements.splice(i, 1);
      }else{
        idx = newLayouts[IDX].columns.findIndex((c) => c.id === colID);
  ...
  const SortableColumnItem = ({ id, containerId, elementData, children }) => {
    ...
    return (
      <div
        ...
        {...listeners}
        style={style}
        className={`column-area col-span-${size} relative`}
        id={`${containerId}/${id}`}
        data-drop="COLUMN"
        onMouseMove={(e) => {
          scheduleBTNUpdate(e);
      ...
      >
        ...
        <div
          className={`column-area border-[1px]  border-dashed border-gray-600 flex-1 ${
        ...
        >
          {hover === id && !activeID && (
            <div className={`z-20 absolute`} onMouseEnter={() => setHover(id)}>
              <OptionButtonGroup
      ...
      >
        <div
          className={`border-[1px]   ${
            elements ||
            (dropTargetRef.current.index?.colI[0] === idx &&
              dropTargetRef.current.index?.colI[1] === index &&
              dropTargetRef.current.index?.conI === IDX &&
              hugeElementType.includes(preview?.type)) ||
            isSpan
              ? "min-h-[40px]"
  ...
  const SortableElementItem = ({
    ...
    const {
      ...
      id,
      data: { type: "ELEMENT", conID: containerId, colID: columnId ,spanID:spanId?spanId:null},
      animateLayoutChanges: noLayoutAnimWhileSorting,
    });
    const style = {
      transform: CSS.Transform.toString(transform),
          ...
          <div className="relative inline-block w-full">
            ...
            <div
              className={`rounded-[8px] pointer-events-none absolute inset-0 ${
                deleteID?.eleID === id || (deleteID?.eleID === id && deleteID?.colID[1] === spanId) ? "bg-red-500/50" : "hidden"
              }`}
            ></div>
          </div>
        )}
  ...
  const change_element_position = (
    ...
    if(Number.isInteger(oldSpanIndex) && Number.isInteger(newSpanIndex)){
      ...
      const newElements = newLayouts[containerIndex].columns[columnIndex].spans[newSpanIndex].elements
  ...
  const change_element_position_new_column = (
    ...
    let oldElements
    let newElements
    if(Number.isInteger(oldSpanIndex) && oldSpanIndex !== -1){
      oldElements =
    ...
    if(Number.isInteger(newSpanIndex)  && newSpanIndex !== -1){
      newElements =
  ...
  const ColumnPreview = ({ element, id, children }) => {
    ...
    if (isSpan) {
      return (
        <div className={`column-area col-span-${size} `}>
          <div className="border-dashed border-[1px] border-gray-600 p-1 h-[200px] justify-center items-center text-center relative">
            <div className="grid grid-flow-col grid-rows-12 gap-4">
              <div
                className="col-span-6 row-span-6 w-full h-full flex"
                style={{
                  background: color,
                  borderRadius: borderRadius,
                  borderWidth: borderWidth,
  ...
  const findElementIndexForDND = (conI, colI, conID, colID, eleID, active) => {
    let index = layouts[conI].columns[colI].elements.findIndex(
      (e) => e.id === eleID
    );
  ...
  const drag = ({ active }) => {
    ...
    if (id.startsWith("Sec-")) {
      ...
      if(current.spanID){
        span = column.spans.find((s)=>s.id === current.spanID)
      ...
      setActiveID({ conID: current.conID, colID: current.colID, eleID: id ,spanID: current.spanID?current.spanID:null});
    }
  };
  ...
  const during = ({ active, over }) => {
    ...
    if (
      ...
      const oldSpanID = active.data.current.spanID || null
      const newSpanID = over.data.current.spanID || null
      ...
      if (oldColumnID === newColumnID && oldContainerID === newContainerID) {
        ...
        let sidx1 = null
        let newIndex
        if(over.data.current.spanID || active.data.current.spanID){
          const oldSpanID = active.data.current.spanID;
          const newSpanID = over.data.current.spanID;
          sidx1 = layouts[IDX].columns[idx].spans.findIndex(s => s.id === oldSpanID)
          sidx2 = layouts[IDX].columns[idx].spans.findIndex(s => s.id === newSpanID)
          if(sidx1 === -1 && sidx2 === -1) return
          const oldIndex = layouts[IDX].columns[idx].spans[sidx1].elements.findIndex(
            (e) => e.id === active.id
          );
          if(layouts[IDX].columns[idx].spans[sidx2].elements.length === 0){
            newIndex = 0
          }else{
            newIndex = layouts[IDX].columns[idx].spans[sidx2].elements.findIndex(
              (e) => e.id === over.id
            );
          }
        ...
        if(oldSpanID || newSpanID){
          ...
          sidx1 = layouts[IDX].columns[idx1].spans.findIndex(s => s.id === oldSpanID) 
          sidx2 = layouts[IDX].columns[idx2].spans.findIndex(s => s.id === newSpanID) 
          oldIndex = sidx1 >= 0 ? layouts[IDX].columns[idx1].spans[sidx1].elements.findIndex(e => e.id === active.id) : layouts[IDX].columns[idx1].elements.findIndex(e => e.id === active.id)
          const newElementsLength = sidx2 >= 0 ? layouts[IDX].columns[idx2].spans[sidx2].elements.length : layouts[IDX].columns[idx2].elements.length


          if (newElementsLength === 0) {
            change_element_position_new_column(oldIndex, 0, IDX, idx1, idx2,sidx1,sidx2);
            return;
          } 
          else {
            const newIndex = findElementIndexForDND(
              IDX,
              idx2,
              newContainerID,
              newColumnID,
                  ...
                  return (
                    <React.Fragment key={ID}>
                      ...
                      >
                        ...
                        >
                          {columns.map((col, i) => {
                            const { id, elements, spans, isSpan } = col;
                            const eleID = elements.map((e) => e.id) ?? [
                              "ele-null",
                            ];

                            setSpanID(spans, ID, id);
                            const spanID = spans.map((s) => s.id);
                              ...
                              >
                                {isSpan && spans.length > 0 ? (
                                  <React.Fragment>
                                    ...
                                    >
                                      {spans.map((span, o) => {
                                        const { id: sID, elements } = span;
                                        const eleForSpnID = elements.map(
                                          (e) => e.id || null
                                        ) ?? ["ele-spn-null"];
                                        return (
                                          <SortableSpanItem
