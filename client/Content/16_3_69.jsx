  ...
  const Content = ({
    ...
    useEffect(() => {
      const handleDeleteElement = (e) => {
        ...
        if (key === "Delete" || key === "Backspace") {
          if (selectID.status !== "Delete" && !selectID.ids) return;
    ...
    useEffect(() => {
      ...
      if (type === "null") {
        return (
          <Box
            ...
            {...listeners}
            style={style}
            id={`${containerId}/${columnId}${spanId?`/${spanId}${miniSpanId?`/${miniSpanId}`:""}`:""}`}
            data-drop="ELEMENT"
      ...
      return (
        ...
        >
          <Element theme={theme} ids={ids} elementData={elementData} selected={selected} isHover={hoverElement.id === id} hover={setHoverElement}/>
          {type === "img" && (
