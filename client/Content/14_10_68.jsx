import React, { useEffect, useMemo, useState, useRef, use, act } from "react";
import {
  Settings,
  Plus,
  Copy,
...
const Content = ({
  ...
  // useState
    ...
    // Drag&Drop
      const [isDragging,setIsDragging] = useState(false) // เก็บค่าสถานะการ Drag&Drop (true = กำลังทำ / false = w,jwfhme)
    // Disable Drag&Drop
  ...
  const updateDND = (x, y) => {
    ...
    const column = el?.closest(`[data-drop="COLUMN"]`);
    const element = el?.closest(`[data-drop="ELEMENT"]`);
    const btnGroup = el?.closest(`[data-drop="COLUMN-BTN"]`);
    ...
    if(btnGroup && column){
        ...
        const isHoverBTNdrag = el.closest("drag-column")
        if(isHoverBTNdrag){
          setDisableConDrag(true)
        setDisableEleDrag(true)
        }
  ...
  const handleDrop = (e)=>{
    ...
    if (dropTargetRef.current.type === "SECTION") dropNewSection(e);
    else dropNewElement(e);
  }
  ...
  const setDrop = (i, t,b = false, id=null) => {
  ...
  const checkGhostPosition = (x, y, r) => {
    return (
      r && x && x >= r.left && x <= r.right && y && y >= r.top && y <= r.bottom
    );
  };
  ...
  const findElement = (col,x,y)=>{
    const nodes = col.querySelectorAll("[data-drop='ELEMENT']")
  ...
  const updateHoverFromPoint = (x, y, type, element) => {
    ...
    if (type === "SECTION") {
      ...
      const EL = findElement(column,x,y)
        ...
        >
          ...
          >
            <div
              ...
              onDragOver={(e) => {
                e.preventDefault();
                const layout = lodash.cloneDeep(handleDropElement());
                if (layout.container) {
                  layout.container.id += page.latestID;
                  for (let i = 0; i < 3; i++) {
                    layout.columns[i].id += `${page.latestID}-${i}`;
                  }
                  scheduleHoverUpdate(e, "SECTION", layout);
                }
              }}
            >
              {children}
      ...
      >
        <div
          className={`column-area border-[1px]  border-dashed border-gray-600 flex ${
            elementData.elements.length > 0 || (dropTargetRef.current.index?.colI === idx && dropTargetRef.current.index?.conI === IDX && hugeElementType.includes(preview?.type))?"min-h-[40px]":"min-h-[200px]"
...
{type === "divider" && (
  <div className={`${isElement?"w-full":"w-[100px]"} w-10 h-[0.5px] my-1`} style={{backgroundColor:"#6a6a6a" }} onMouseEnter={() => setHoverElement({ id: id })}
  onMouseLeave={() => setHoverElement(false)}/>
...
<List dense sx={{ width: '100%',py:0,my:0.5}}>
  ...
  const change_column_position_new_container = (
    oldIndex,
    newIndex,
    oldContainerIndex,
    newContainerIndex
    ...
    setLayout((prev) => {
      const newLayouts = prev.map((l) => ({ ...l, columns: [...l.columns] }));
      const oldColumns = newLayouts[oldContainerIndex].columns;
      const newColumns = newLayouts[newContainerIndex].columns;
      const [column] = oldColumns.splice(oldIndex, 1);
      newColumns.splice(newIndex, 0, column);
      if (oldColumns.length === 0) {
        newLayouts.splice(oldContainerIndex, 1);
      }
      return newLayouts;
    });
  };
  ...
  const ColumnPreview = ({ element, id, children }) => {
    ...
    return (
      <div
        onMouseEnter={() => setHover(colID)}
        onMouseLeave={() => setHover(colID)}
...
/>
  ...
  <> 
      <div className={`w-[100px] w-10 h-[0.5px] bg-red-500 my-1`} style={{backgroundColor:"#6a6a6a" }}/>
  </>
)}
{element.type === "btnG" && (
...
<List dense sx={{ width: '100%',py:0,my:0.5}} ref={(el)=>setDragRef(el)}>
    ...
    if (
      ...
      if (
        ...
        if (oldContainerID === newContainerID) {
          ...
          const oldIndex = layouts[IDX1].columns.findIndex(
            (c) => c.id === active.id
          );
          let newIndex = layouts[IDX2].columns.findIndex(
            (c) => c.id === over.id
          );
          if (oldIndex === -1) return;
          if (newIndex === -1) newIndex = layouts[IDX2].columns.length;
          if (oldIndex === newIndex) return;
          change_column_position_new_container(oldIndex, newIndex, IDX1, IDX2);
          return;
        }
      }
    ;
  ...
  //   const filtered = droppableContainers.filter((dc) => {
  //     const t = dc.data.current.type;
  //     return type === "ELEMENT" ? (t === "ELEMENT" || t === "COLUMN"): (t === type)
  //   });

  //   const detector = type === "ELEMENT"
  //   ? pointerWithin
  //   : closestCenter;
  //   return detector({...args,droppableContainers:filtered})
  ...
  return (
    ...
    >
      ...
      <DndContext
        onDragStart={(e)=> {addClass();drag(e)}}
        onDragMove={(e)=> during(e)} 
        onDragEnd={(e)=>{drop(e);removeClass()}}
        sensors={sensors}
        autoScroll
        measuring={measuring}
            ...
            {layouts.length > 0 ? (
              ...
              <>
                {preview && (
                  <div
                    ref={ghostRef}
                    className="preview opacity-70 pointer-events-none"
                    key="ghost-end1"
                    data-drop="SECTION"
                    id={preview.container.id}
                  >
          ...
          >
            {activeID &&
              ...
              ((typeof activeID === "string" && (
                ...
                (typeof activeID === "object" && activeID.eleID && (
                  <ElementPreview element={activeItem}></ElementPreview>
                )))}
          </DragOverlay>
        </div>
      </DndContext>
